...
%-------------------- d_dot
        ...
        if (P_mod(i)==0) && (p<iso_par.d0) && (p_dot<0)
            ...
            if ((P_mod(i)==O3)) && (d>P_d_star(i)+1) && (sign(d_dot)~=sign(iso_save.d_dot_old(i)))% && (d_dot>0)
                P_d_star(i)=d;
                P_mod(i)=2;
                Cmod=0;
            else
                ...
                if (Cmod==0) && (P_mod(i)==2) && (d>=P_d_star(i)+iso_par.dD) && (d_dot>=-0.01)
                    P_mod(i)=0;
                    P_d_star(i)=NaN;
                    P_p_star(i)=NaN;
                end
...
if (Modul.N==3)
    ...
    hold on
%     Save_iso.plot_p_xi=plot(0,0,'R','LineWidth',2);
    legend('p''','Location','NorthWest');

%     legend('p''','Xi','Location','NorthWest');
...
% addPlotData(Save_iso.plot_p_xi,p_xi);
