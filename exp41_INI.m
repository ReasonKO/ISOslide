...
iso_par.ExpName='Dynamic_2';
iso_par.re_D=@(x,y)exp3_reD(x,y);
iso_par.Dynamic=@exp41_dynamic;
...
Z=1*or(abs(X)>50,Y>10);
field.Xm{1}=X;
field.Ym{1}=Y;
field.Zm{1}=Z;

[X,Y]=meshgrid(-30:1:40,-50:1:30);

field.Xm{2}=X;
field.Ym{2}=Y;
field.Zm{2}=1*or(abs(X)<5,abs(Y)<5);

[R,ang]=meshgrid(75:1:80,-pi/4+(-pi:pi/100:1.5*pi/2));
