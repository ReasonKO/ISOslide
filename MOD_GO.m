...
ad=sign(Right-Left)*abs(a2-a1)/2;
if (viz) 
    if (get(0,'CurrentFigure')==100)
        global MAP_PAR;
        if (isfield(MAP_PAR,'Track') &&(length(get(MAP_PAR.Track,'xdata'))<20000))
            xd=get(MAP_PAR.Track,'xdata');
            yd=get(MAP_PAR.Track,'ydata');
        %   set(MAP_PAR.Track,'xdata',[xd(max(end-2000,1):end),CC(1)+Xr*cos(a1:ad:a2)],'ydata',[yd(max(end-2000,1):end),CC(2)+Xr*sin(a1:ad:a2)]);            
            set(MAP_PAR.Track,'xdata',[xd(1:end),CC(1)/100+Xr*cos(a1:ad:a2)/100],'ydata',[yd(1:end),CC(2)/100+Xr*sin(a1:ad:a2)/100]);            
        else            
            fprintf('nettr\n');
            MAP_PAR.Track=plot(CC(1)/100+Xr*cos(a1:ad:a2)/100,CC(2)/100+Xr*sin(a1:ad:a2)/100,'color',treckcolor,'LineWidth',1.2);
        end
    end
end
Xn=(LnC+RnC)/2;
